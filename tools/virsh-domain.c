...
cmdLimitDirtyPageRate(vshControl *ctl, const vshCmd *cmd)
{
    ...
    if (vcpu) {
        ...
        if (vcpu_idx < 0) {
            vshError(ctl, "%s",
                     _("Invalid vcpu index, using --vcpu to specify cpu index"));
            return false;
        }
    }
    ...
    if (vcpu) {
        ...
        if (rate == 0)
            vshPrintExtra(ctl,
                          _("Cancel vcpu[%1$d] dirty page rate upper limit successfully\n"),
                          vcpu_idx);
        else
            vshPrintExtra(ctl,
                          _("Set vcpu[%1$d] dirty page rate upper limit %2$lld(MB/s) successfully\n"),
                          vcpu_idx, rate);
    } else {
        ...
        if (rate == 0)
            vshPrintExtra(ctl, "%s",
                          _("Cancel dirty page rate limit for all virtual CPUs successfully\n"));
        else
            vshPrintExtra(ctl,
                          _("Set dirty page rate limit %1$lld(MB/s) for all virtual CPUs successfully\n"),
                          rate);
    }
